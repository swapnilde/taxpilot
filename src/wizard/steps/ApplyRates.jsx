...
export default function ApplyRates({
	...
	const getApplyButtonLabel = () => {
		...
		return `✓ ${action} ${count} ${__('Rates', 'taxpilot-for-woocommerce')}`;
	};
	...
	const handleApply = async () => {
		...
		try {
			...
			showNotice(
				__('Failed to apply rates. Please try again.', 'taxpilot-for-woocommerce'),
				'error'
			);
		} finally {
			setApplying(false);
	...
	if (result?.success) {
		return (
			<div className="taxpilot-success">
				...
				<h2>{__('Tax Setup Complete!', 'taxpilot-for-woocommerce')}</h2>
				<p>
					{result.applied}{' '}
					{__(
				...
				{result.errors?.length > 0 && (
					<div style={{ marginBottom: 'var(--tw-space-4)' }}>
						<span className="taxpilot-badge taxpilot-badge--warning">
							...
							{__('errors occurred', 'taxpilot-for-woocommerce')}
						</span>
						<ul
							style={{
								textAlign: 'left',
				...
				>
					...
					>
						{__('Go to Dashboard →', 'taxpilot-for-woocommerce')}
					</a>
					<a
						href={
							window.taxPilotData?.adminUrl +
					...
					>
						{__('View WooCommerce Tax Settings', 'taxpilot-for-woocommerce')}
					</a>
					{onStartOver && (
						<button
							...
							onClick={onStartOver}
						>
							{__('🔄 Start Over', 'taxpilot-for-woocommerce')}
						</button>
					)}
				</div>
			</div>
		...
		return (
			<div className="taxpilot-loading">
				...
				<span className="taxpilot-loading-text">
					{__('Loading rate preview…', 'taxpilot-for-woocommerce')}
				</span>
			</div>
		);
	}
	...
	return (
		<div>
			...
			<div className="taxpilot-apply-summary">
				...
				<div className="taxpilot-apply-stat">
					<span className="taxpilot-apply-stat-label">
						{__('Tax Rates to Apply', 'taxpilot-for-woocommerce')}
					</span>
					<span className="taxpilot-apply-stat-value">
						{displayRateCount}
					</span>
