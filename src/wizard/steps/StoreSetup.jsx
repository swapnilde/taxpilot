...
export default function StoreSetup({
	...
	const countryOptions = [
		{ value: '', label: __('— Select Country —', 'taxpilot-for-woocommerce') },
	...
	const currencyOptions = [
		{ value: '', label: __('— Select Currency —', 'taxpilot-for-woocommerce') },
	...
	const handleNext = async () => {
		if (!data.country || !data.currency) {
			showNotice(
				__('Please select both country and currency.', 'taxpilot-for-woocommerce'),
				'error'
			);
			return;
		}
		...
		try {
			...
			showNotice(__('Store setup saved!', 'taxpilot-for-woocommerce'));
			onNext();
		} catch {
			showNotice(
		...
		return (
			<div className="taxpilot-loading">
				...
				<span className="taxpilot-loading-text">
					{__('Detecting store settings…', 'taxpilot-for-woocommerce')}
				</span>
			</div>
		);
	}
		...
		<div>
			...
			<div className="taxpilot-form-row">
				<div className="taxpilot-field">
					<SelectControl
						label={__('Store Country', 'taxpilot-for-woocommerce')}
						value={data.country}
				...
				<div className="taxpilot-field">
					<SelectControl
						label={__('Store Currency', 'taxpilot-for-woocommerce')}
						value={data.currency}
